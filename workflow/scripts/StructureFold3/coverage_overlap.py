...
Python 3 port of StructureFold2's deprecated/coverage_overlap.py. Transcripts are written sorted by name (Python 2 wrote
them in arbitrary set order); the summary lists the files in the order Python 2 printed them.
'''

#Imports
import glob
import argparse
from sf3libs import py2_dict_order

def read_coverage(coverage_file):
    '''Reads a coverage <.csv> into {column name: {transcript: coverage}}'''
    all_the_things = {}
...
def write_flat_list(master_dictionary,xthreshold=1):
    ...
    file_keys = sorted(master_dictionary.keys())
    outname ='_'.join([group.replace('_coverage','') for group in file_keys]+['overlap',str(xthreshold)])+'.txt'
    common_keys = set([a for b in [sub.keys() for sub in master_dictionary.values()] for a in b])
    with open(outname,'w') as g:
        for key in sorted(common_keys):
            try:
                test_list = [(file_key,master_dictionary[file_key][key]) for file_key in file_keys]
    ...
    print('')
    for filename in py2_dict_order(master_dictionary):
        subdict = master_dictionary[filename]
