#!/usr/bin/env python3
#StructureFold2 batch running script for Bowtie2 (Python 3 port)
...
def bowtie_batcher(infile,mapping_index,threads_number,out_suffix,phred_adjust,multimap,fail_reads,log_flag):
    ...
    if log_flag == False:
        ...
        return None
    # bowtie2 writes its alignment summary to stderr; capture it (stdout goes to the SAM file)
    proc = subprocess.run(command, shell=True, stdout=subprocess.PIPE,
                          stderr=subprocess.STDOUT, universal_newlines=True)
    return proc.stdout,outfile,command
