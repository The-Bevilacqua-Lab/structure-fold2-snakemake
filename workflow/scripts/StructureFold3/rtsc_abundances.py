...
Python 3 port. Output values are formatted with Python 3's shortest round-trip float repr
(Python 2's str() kept only 12 significant digits).
'''
...
def read_in_total_stops(afile):
    ...
    with open(afile,'r') as f:
        #Each record is a transcript line, a tab-separated counts line, and a blank line
        lines = (line.strip() for line in f)
        lines = (line for line in lines if line)
        for transcript,stops in zip(lines,lines):
            tokens = stops.split('\t')
            try:
                #Counts are integers and mostly 0, so only convert the non-zero tokens
                total = float(sum(map(int,[x for x in tokens if x != '0'])))
            except ValueError:
                total = sum([float(x) for x in tokens])
            information[transcript] = (total,len(tokens))
    return information
