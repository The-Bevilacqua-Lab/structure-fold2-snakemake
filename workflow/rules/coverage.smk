...
rule rtsc_end_coverage_sweep:
    """Mean 3' coverage for trim3 values 0-200 (on demand)."""
    input:
    ...
    params:
        workdir=f"{workflow.basedir}/workflow",
        script="scripts/StructureFold3/rtsc_end_coverage.py",
        length=50,
        tp_l=300,
        max_trim=200,
    shell:
        """
        mkdir -p $(dirname {output}) \
            && python3 {params.workdir}/{params.script} {input} TP \
                -length {params.length} -tp_l {params.tp_l} -sweep {params.max_trim} \
                -name {output} >{log} 2>&1
        """


rule plot_end_coverage_sweep:
    """Plot the trim3 sweep (on demand)."""
    input:
        f"{config['output_dir']}/{{id}}/end_coverage_trim_sweep.tsv",
    output:
        report(
